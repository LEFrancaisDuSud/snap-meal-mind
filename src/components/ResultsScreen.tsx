...
import type { NutritionData, PortionMultiplier, IngredientItem, MealType } from "@/types/nutrition";
...
export default function ResultsScreen({
  imageBase64, data, setData, portionMultiplier, setPortionMultiplier, onLogged, onRetake, saving,
}: Props) {
  ...
  const healthColor =
    data.health_score >= 7 ? "hsl(var(--primary))"
      : data.health_score >= 4 ? "hsl(var(--tertiary))"
      : "hsl(var(--destructive))";
  ...
  return (
    <div className="absolute inset-0 bg-background overflow-hidden flex flex-col">
      ...
      <div className="relative w-full shrink-0" style={{ height: 240 }}>
        <img src={imageBase64} alt={data.dish_name} className="absolute inset-0 w-full h-full object-cover rounded-b-3xl" />
        <div
          className="absolute inset-0 rounded-b-3xl"
          style={{ background: "linear-gradient(to bottom, rgba(0,0,0,0.45) 0%, rgba(0,0,0,0) 35%, rgba(0,0,0,0) 60%, hsl(var(--background) / 0.9) 100%)" }}
        />
        <button
          onClick={onRetake}
          className="absolute top-4 left-4 w-10 h-10 rounded-full bg-black/40 backdrop-blur-md flex items-center justify-center text-white"
          aria-label="Retour"
        >
          <ArrowLeft size={18} />
        </button>
        {pills.map((c, i) => {
          ...
          return (
            <motion.div
              ...
              className="absolute bg-white/95 text-black text-xs font-medium px-2.5 py-1 rounded-full shadow-md max-w-[55%] truncate"
              style={style}
            >
              {c.emoji} {c.name}
            </motion.div>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide px-5 pt-3 pb-40">
          <div className="flex items-center gap-3 relative">
            <h2 className="text-foreground font-bold text-xl truncate flex-1">{data.dish_name}</h2>
            <button
              onClick={() => setMealTypeOpen((o) => !o)}
              className="bg-primary/15 text-primary text-xs font-semibold px-3 py-1.5 rounded-full whitespace-nowrap"
            >
              {MEAL_EMOJI[data.meal_type]} {data.meal_type}
            </button>
            {mealTypeOpen && (
              <div className="absolute right-0 top-9 z-30 bg-card border border-border rounded-2xl p-1 shadow-2xl">
                {MEAL_TYPES.map((t) => (
                  <button
                    key={t}
                    onClick={() => { setData({ ...data, meal_type: t }); setMealTypeOpen(false); }}
                    className={`w-full text-left px-3 py-2 text-sm rounded-xl ${
                      data.meal_type === t ? "bg-primary/20 text-primary font-bold" : "text-foreground hover:bg-surface"
                    }`}
                  >
                    {MEAL_EMOJI[t]} {t}
                  </button>
          ...
          <div className="my-5 text-center">
            <div className="text-foreground font-bold leading-none" style={{ fontSize: 56 }}>{animatedCals}</div>
            <div className="text-primary text-lg font-semibold mt-1">kcal</div>
          </div>

          <div className="grid grid-cols-3 gap-2">
            <MacroCard color="hsl(var(--macro-protein))" label="Protéines" value={macroValues.protein} />
            <MacroCard color="hsl(var(--macro-carbs))" label="Glucides" value={macroValues.carbs} />
            <MacroCard color="hsl(var(--macro-fat))" label="Lipides" value={macroValues.fat} />
          </div>

          <div className="mt-6">
            <div className="flex items-center justify-between">
              <span className="text-foreground font-semibold text-sm">Score santé</span>
              <span className="font-bold text-sm" style={{ color: healthColor }}>{data.health_score}/10</span>
            </div>
            ...
            {data.health_tip && (
              <p className="text-muted-foreground italic text-sm mt-2 leading-relaxed">{data.health_tip}</p>
            )}
          </div>

          <h3 className="text-foreground font-bold mt-6 mb-3">Ingrédients détectés</h3>
          <div className="space-y-2">
            {data.components.length === 0 && (
              <div className="text-muted-foreground text-sm bg-card rounded-2xl p-4">Aucun ingrédient détecté.</div>
            )}
            {data.components.map((c, i) => {
              ...
              return (
                <motion.div
                  key={i}
                  animate={flashIndex === i ? { backgroundColor: ["hsl(var(--primary) / 0.3)", "hsl(var(--card))"] } : {}}
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-muted-foreground text-xs mt-0.5 truncate">{c.quantity_estimate}</div>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      <div className="text-foreground font-bold text-sm whitespace-nowrap">
                        {Math.round(c.calories * portionMultiplier)} kcal
                      </div>
                      <button onClick={() => setEditingIndex(i)} className="text-muted-foreground hover:text-foreground p-1" aria-label="Modifier">
                        <Pencil size={16} />
                      </button>
                    </div>
                  </div>
          ...
          <div className="mt-6">
            <div className="text-muted-foreground text-sm mb-2">Taille de portion</div>
            <div className="flex gap-2">
              {PORTIONS.map((p) => {
                const selected = p === portionMultiplier;
                return (
                  <motion.button
                    key={p} whileTap={{ scale: 0.92 }} whileHover={{ scale: 1.03 }}
                    onClick={() => setPortionMultiplier(p)}
                    className={`flex-1 py-2.5 rounded-full text-sm font-semibold transition-colors ${
                      selected ? "bg-primary text-primary-foreground" : "bg-card text-muted-foreground border border-border"
                    }`}
                  >{p}x</motion.button>
                );
              })}
        ...
        >
          <div className="pointer-events-auto space-y-2">
            ...
            <motion.button
              whileTap={{ scale: 0.97 }} onClick={onRetake}
              className="w-full border border-border text-foreground py-4 rounded-full"
            >
              📷 Reprendre la photo
            </motion.button>
          </div>
...
function MacroCard({ color, label, value }: { color: string; label: string; value: number }) {
  return (
    <div className="bg-card rounded-2xl p-3 text-center relative overflow-hidden">
      <div className="absolute top-0 left-0 right-0" style={{ height: 3, background: color }} />
