...
import { Bell, Flame, Shield, Check, Plus, ChevronRight } from "lucide-react";
...
import { todayISO, isoOf, shortDay } from "@/lib/dates";
...
export default function HomePage() {
  ...
  const weekDays = Array.from({ length: 7 }, (_, i) => {
    ...
    d.setDate(today_d.getDate() - today_d.getDay() + 1 + i); // Mon..Sun
    return d;
  });

  const streakDays = profile?.current_streak || 0;
  const isCompleted = (idx: number) => {
    const dayOfWeek = weekDays[idx].getDay(); // Sun=0
    const todayDow = today_d.getDay();
    // Mark days from (today - streak + 1) up to today
    ...
    <div className="absolute inset-0 overflow-y-auto scrollbar-hide">
      ...
      <div className="px-5 pb-4 flex items-center gap-3">
        ...
        <div className="bg-tertiary/15 text-tertiary px-3 py-1.5 rounded-full font-bold text-sm flex items-center gap-1">
          <Flame size={14} fill="currentColor" /> {streakDays}
        </div>
      </div>
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-baseline gap-2">
            <span className="text-primary font-extrabold text-lg leading-none">NIVEAU {xpInfo.level}</span>
          </div>
          <div className="text-foreground font-semibold text-sm mt-0.5">Guerrier Nutritionnel</div>
          <div className="flex items-center justify-between text-xs mt-2">
            <span className="text-muted-foreground">Prochain niveau : {xpInfo.nextLevelAt} XP</span>
        ...
        <div className="flex justify-center mt-3">
          ...
          >
            <div className="text-center">
              <div className="text-foreground font-extrabold leading-none" style={{ fontSize: 38 }}>
                {animatedRemaining}
              </div>
      ...
      >
        <div className="flex items-baseline justify-between">
          <div>
            ...
            <div className="text-primary text-sm font-semibold">{streakDays} jours consécutifs</div>
          </div>
          <div className="text-right">
            <div className="text-muted-foreground text-[10px] font-bold uppercase tracking-widest">Record</div>
            <div className="text-foreground font-extrabold">{profile?.best_streak || streakDays}j</div>
          </div>
        </div>
      ...
      <div className="mx-5 mt-5 flex items-center justify-between">
        ...
        <button className="text-primary text-xs font-bold tracking-wider">VOIR TOUT</button>
      </div>
                ...
                {empty ? (
                  ...
                  <>
                    ...
                    <div className="text-primary font-bold text-sm mt-0.5">
                      {cal} kcal <span className="text-muted-foreground font-normal">• COMPLET</span>
                    </div>
                  </>
                )}
              </div>
...
function MacroBar({ label, value, target, color }: { label: string; value: number; target: number; color: string }) {
  ...
  return (
    <div className="text-center">
      <div className="text-[10px] font-bold tracking-wider" style={{ color }}>{label}</div>
